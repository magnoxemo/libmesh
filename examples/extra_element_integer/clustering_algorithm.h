  ...
  for (const auto &elem : mesh.element_ptr_range()) {
    if (elem->get_extra_integer(index) == elem->id()) {
      for (unsigned int side = 0; side < elem->n_sides(); side++) {
        ...
        if (neighbor_elem) {
          if (BelongToCluster(elem, neighbor_elem, variable_num, system)) {

            std::vector<libMesh::Elem *> neighbor_list;
            neighbor_elem->set_extra_integer(index, elem->id());
            neighbor_list.push_back(neighbor_elem);

            std::cout << "main_element_id = " << elem->id()
                      << " cluster_elem_id = " << neighbor_elem->id()
                      << std::endl;

            while (neighbor_list.size() > 0) {
              libMesh::Elem *current_elem = neighbor_list.back();
              ...
              for (unsigned int s = 0; s < current_elem->n_sides(); s++) {
                ...
                if (neighbor_elem &&
                    neighbor_elem->get_extra_integer(index) != elem->id() &&
                    (BelongToCluster(current_elem, neighbor_elem, variable_num,
                                     system))) {
                  neighbor_elem->set_extra_integer(index, elem->id());
                  std::cout
                      << "new element to the cluster = " << neighbor_elem->id()
                      << " derived from = " << current_elem->id() << std::endl;
                  neighbor_list.push_back(neighbor_elem);
                }
              }
            }
      ...
      std::cout << "skipping element " << elem->id() << std::endl;
    }
  }
