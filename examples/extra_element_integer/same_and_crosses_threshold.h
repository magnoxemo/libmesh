...
bool BelongToCluster(double elem_solution, double neighbor_solution) {
    ...
    if ((static_cast<int>(elem_solution)>THRESHOLD && static_cast<int>(neighbor_solution)>THRESHOLD) &&(static_cast<int>(elem_solution)==static_cast<int>(neighbor_solution))) {
        return true;
    } else {
        return false;
    }
